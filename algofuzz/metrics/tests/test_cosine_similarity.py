...
import os
import numpy as np
from algofuzz.metrics.distance_pairwise import cosine_similarity
...
def test_2d_cosine_similarity():
    ...
    np.testing.assert_allclose(
        cosine_similarity(point1, point2), cosine_similarity(point1, point2), atol=1e-9)
...
def test_3d_cosine_similarity():
    ...
    np.testing.assert_allclose(
        cosine_similarity(point1, point2), np.array([[0.97463185]]), atol=1e-9)
...
def test_same_point_cosine_similarity():
    ...
    np.testing.assert_allclose(
        cosine_similarity(point1, point2), np.array([[1]]), atol=1e-9)
...
def test_large_numbers_cosine_similarity():
    ...
    np.testing.assert_allclose(
        cosine_similarity(point1, point2), np.array(cosine_similarity(point1, point2)), atol=1e-7)
...
def test_benchmark_distance_np(benchmark, vectors):
    ...
    benchmark(sklearn_cosine, vector_a, vector_b)
